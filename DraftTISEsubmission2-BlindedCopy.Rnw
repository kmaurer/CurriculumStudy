...
\author{Author 1 \\ Institution X\\
        Author 2 \\ Institution X }
...
%\hh{XXX this might prove to be problematic: Dennis and you randomly assigned students to the treatment groups. But THEN students interacted with each other, which is introducing the deviation from independence. Students between treatments can be assumed to interact less, at least once you split the groups into two classrooms. The simulation should reflect that. One way to do that, would be to set the covariance between students from different treatments to 0 (or at least reduce it ). Try it for a couple of simulation runs, and see what an effect it has. }
