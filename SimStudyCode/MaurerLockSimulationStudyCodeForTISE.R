...
#------------------------------------------------------------------------------------------------------
### The original class scores data is not available due to FERPA regulations concerning student grades 
# consequently, you will need to skip down to line 204 in order to recreate study from provided simulation results data
...
### Try small Simulation for  tau=.5, gamma=.05, delta=.05, eta=.4
timer <- Sys.time()
set.seed(62668)
M= 100
truemod <-  lm(cbind(ConfMC,HypMC)~ midterm + lab5perc, data=dat)
...
# Check error rate for small sim:
...
# setwd(" --- Set Working Directory --- ")
# save(resultlist,shortsim, file = "NewSimulationResultsReparam.RData")

#----------------------------------------------------------------------------
# Calculate and store error rates in simulations

load("NewSimulationResultsReparam.Rdata") # 130 Mb of fitted model simulation results
# Take a peek at results
str(resultlist[[1]])
head(shortsim)

### Compute error rates from simulations
L <- length(resultlist)
M = nrow(resultlist[[1]]) 
