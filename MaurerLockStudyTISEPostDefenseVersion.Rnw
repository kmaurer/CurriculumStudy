...
\end{eqnarray}
%
where $y_{ijkl}$ is the $\ell^{th}$ response from student $i$ who is in lab group $j$ and classroom $k$ for $i \in \left\{1,\dots,n\right\}$, $j \in \left\{1,2,3,4\right\}$, $k \in \left\{1,2\right\}$, and $\ell \in \left\{1,2\right\}$. $\beta_{\ell 0}$, $\beta_{\ell 1}$ and $\beta_{\ell 2}$ are the fixed effect coefficients for response $\ell$ for the common intercept, lab 5 score and midterm score, respectively. In the generative model these coefficients are set equal to the estimated coefficients for the intercept, lab 5 score and midterm score from the original ARTIST model. The remaining additive terms from the model are random effects with the distributions defined such that
%
% \begin{tabular}{lp{5in}}
...
\]
%
Thus the variance structure for the responses includes common variance components $\sigma_{\ell\ell'}$ and scaling parameters $z$, $g$ and $d$. The variance components $\sigma_{\ell\ell'}$ values estimated from the original ARTIST model are used as plug in estimates to be used in the generative model. The scaling parameter $z$ controls the variability that is common between all students; thus $z \sigma_{\ell\ell'}$ is the baseline covariance of responses for students from different labs. The scaling parameter $g$ controls the additive increase to covariance between students of the same lecture section, $g \sigma_{\ell\ell'}$, and $d$ controls the additive increase to covariance between students of the same lab section, $d \sigma_{\ell\ell'}$. If each of these variance scaling parameters is set to zero, there is no violation to the assumption of independence between students; thus making the error structure of the generative model~\ref{eq:simgenerative} match the error structure of the ARTIST model~\ref{eq:mancova}. However, these parameters may be increased to violate the independence assumption to varying degrees.

The simulation procedure for assessing the error rates under a violation independence between student responses is conducted in a five step process. We consider with violations with all combinations of $d$ in $\{0,0.02,0.04,0.06,0.08,0.1\}$ and $g$ in $\{0,0.02,0.04,0.06,0.08,0.1\}$. We set $z$=0 for all simulations, as a random effect that is common to \textit{all} students will not effect tests for a curriculum effect. For each combination of variance scaling parameters we repeat the following simulation process for $M = 20,000$: 
%
\begin{enumerate}
...
Recall that in the generative model~(\ref{eq:simgenerative}) does not include a treatment effect, therefore any simulation where significant curriculum effects are found is a Type 1 error. Figure~\ref{fig:simerrorratesdg} displays the observed type 1 error rates under violations of between student independence. When the variance scaling parameters are set to zero there is no violation of independence and we see that the tests hold at the nominal $\alpha$=0.05 Type 1 error rate. We see that the Type 1 error rates increase quickly when the variance scaling parameters, $d$ and $g$ increase; most dramatically for Pillai's test. 

When the between labmate covariance in 4\% higher than for non-labmates (i.e. $d$=0.04) the Type 1 error rates for individual t-tests are above 0.15 and the Pillai's test is above 0.20; over three and four times the nominal rate, respectively. Even worse, when the between classmate covariance in 4\% higher than for non-classmates (i.e. $g$=0.04) the Type 1 error rates for individual t-tests are above 0.25 and the Pillai's test is above 0.35; over five and seven times the nominal rate, respectively. This error rate inflation occurs because the random effects based on class and lab sections are being misinterpreted in the ARTIST model as fixed effects of curriculum due to the assumption of independence between students; a misinterpretation made worse in the case of class section due to the complete confounding with curriculum.
}
...
\km{
Note that this covariance disparities would need to be attributed to non-curricular factors because the generative model is designed to carry no curriculum treatment effect. Great care was taken during the study design and administration to minimize all non-treatment related differences that may have arisen due to the physically different class and lab rooms; using the alternation of instruction, identical curricula administered with all students in the same room in weeks 1 to 8, and careful pedagogical preparation. However, the simulation study indicates that even in the case of very small disparities in class or lab section based covariances between students we suffer from inflations to the Type 1 error rates raises major doubts on the results in the model based analyses in Sections~\ref{ArtistModel} and~\ref{AppliedModel}. 
}
