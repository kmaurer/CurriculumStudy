...
\km{
The distributions for the random effects and error terms is parameterized such that
\[
\vec{\epsilon}_{ijk} = 
\begin{bmatrix}
...
\]
%
Thus the variance structure for the responses includes common variance components $\sigma_{\ell\ell'}$ and scaling parameters $z$, $g$ and $d$. The variance components $\sigma_{\ell\ell'}$ values estimated from the original ARTIST model are used as plug in estimates to be used in the generative model. If each of these variance scaling parameters is set to zero, there is no violation to the assumption of independence between students; thus making the error structure of the generative model~\ref{eq:simgenerative} match the error structure of the ARTIST model~\ref{eq:mancova}. However, these parameters may be increased to violate the independence assumption to varying degrees.

The scaling parameter $z$ controls the variability that is common between all students; thus $z \sigma_{\ell\ell'}$ is the baseline covariance of responses for students from different labs. The scaling parameter $g$ controls the additive increase to covariance between students of the same lecture section and $d$ controls the additive increase to covariance between students of the same lab section.}
